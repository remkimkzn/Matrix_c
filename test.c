...
START_TEST(create_INCORRECT_MATRIX) {
  int rows = -10;
  int columns = 10;
  matrix_t matrix = {0};
...
START_TEST(sub_test_INCORRECT_MATRIX_matrix) {
  matrix_t A, B, R;
  s21_create_matrix(1, 1, &A);
  s21_create_matrix(2, 1, &B);
...
START_TEST(sum_test_INCORRECT_MATRIX_matrix) {
  matrix_t A, B, R;
  s21_create_matrix(1, 1, &A);
  s21_create_matrix(2, 1, &B);
...
START_TEST(inverse_normal) {
  ...
  if (!codec1 && !codec2) {
    ...
    printf("Code: %d\n", code);  // Вывод кода возврата
    printf("Result matrix:\n");
    // Вывод результата (полезно для отладки)
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        printf("%.6f ", result.matrix[i][j]);
      }
      printf("\n");
    }
...
START_TEST(simple_inverse_test) {
  matrix_t A, B;
  s21_create_matrix(2, 2, &A);
  s21_create_matrix(2, 2, &B);
...
START_TEST(determinant_test) {
  ...
  ck_assert_double_eq_tol(det, 11.4, 1e-6);
...
START_TEST(calc_complements_test) {
  ...
  int res = s21_calc_complements(&A, &complements);
...
START_TEST(transpose_test) {
  ...
  ck_assert_double_eq_tol(B.matrix[1][1], 5.0, 1e-6);
...
START_TEST(create_matrix_test) {
  ...
  int res = s21_create_matrix(2, 3, &result);
...
Suite
  ...
  tcase_add_test(tCase, create_INCORRECT_MATRIX);
  ...
  tcase_add_test(tCase, sub_test_INCORRECT_MATRIX_matrix);
  ...
  tcase_add_test(tCase, sum_test_INCORRECT_MATRIX_matrix);
  ...
  tcase_add_test(tCase, transpose_INCORRECT_MATRIX);
  ...
  tcase_add_test(tCase, inverse_INCORRECT_MATRIX);
  tcase_add_test(tCase, inverse_one_by_one);
  tcase_add_test(tCase, simple_inverse_test);
  tcase_add_test(tCase, determinant_test);
  tcase_add_test(tCase, calc_complements_test);
  tcase_add_test(tCase, transpose_test);
  tcase_add_test(tCase, create_matrix_test);
